...
export default async function ProductPage({params}:{params:Promise<{slug:string}>}){const {slug}=await params;const p=await getProductBySlug(slug);if(!p||p.isDiscoverySet)notFound();const noteStages=[p.notes.top,p.notes.heart,p.notes.base].filter((layer)=>layer.length).length;const all=await getProducts();const related=all.filter(x=>x.id!==p.id&&!x.isDiscoverySet&&(x.family===p.family||x.notes.base.some(n=>p.notes.base.includes(n)))).slice(0,2);return <div className="pdp" style={{"--accent":p.accent} as React.CSSProperties}>
  ...
  <section className="pdp-story section-shell"><p className="story-index" data-reveal="up">01 / THE SHAPE</p><div className="story-notes" data-reveal="up" data-reveal-delay="0.06">{([["top","Top notes","0—15 min"],["heart","Middle notes","15 min—3 h"],["base","Base notes","3 h+"]] as const).filter(([key])=>p.notes[key].length).map(([key,label,time])=><div key={key}><p className="kicker">{label}<span>{time}</span></p><ul>{p.notes[key].map((note)=><li key={note}><Link href={`/notes/${encodeURIComponent(note.toLowerCase().replaceAll(" ","-"))}`}>{note}</Link></li>)}</ul></div>)}</div><blockquote data-reveal="up" data-reveal-delay="0.12">“{p.performance.wearsLike}.”</blockquote></section>
  <section className="notes-section section-shell"><div className="section-title" data-reveal="up"><p className="kicker">02 / How it unfolds</p><h2>{noteStages===2?"Two":noteStages===1?"One":"Three"} moment{noteStages===1?"":"s"},<br/>not one list.</h2><p>Tap a stage to see what takes the foreground. Each material opens into its own vocabulary page.</p></div><NotePyramid product={p}/></section>
