...
interface RazorpayWebhookEvent { event?: string; created_at?: number; payload?: { payment?: { entity?: { id?: string; order_id?: string } } } }
...
export async function POST(request: Request) {
  ...
  try { event = JSON.parse(raw) as RazorpayWebhookEvent; } catch { return json({ error: "Invalid JSON." }, 400); }
  ...
  if (event.event && ["payment.captured", "order.paid"].includes(event.event)) {
