...
export function CheckoutForm({ products, turnstileSiteKey }: { products: Product[]; turnstileSiteKey?: string }) {
  ...
  async function submit(formData: FormData) {
    ...
    try {
      ...
      if(data.mode==="mock"){
        const verify=await fetch("/api/payments/verify",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({orderId:data.orderId,providerOrderId:data.providerOrderId,paymentId:`mock_payment_${Date.now()}`,signature:"mock_signature"})});
        if(!verify.ok){const p=await verify.json();setError(p.error||"Mock payment verification failed.");return;}
        clearCart(); sendEvent("purchase",{orderId:data.publicId,amountPaise:data.amountPaise}); router.push(`/order/${encodeURIComponent(data.publicId)}?token=${encodeURIComponent(data.guestToken)}`); return;
      }
  ...
  return <div className="checkout-layout"><Script src="https://checkout.razorpay.com/v1/checkout.js" strategy="afterInteractive"/><form className="checkout-form" action={submit}><header><p className="kicker">Secure checkout</p><h1>Where should<br/><em>it arrive?</em></h1><p>Your browser never decides price or stock. We re-check every variant on the server before payment.</p></header>
    ...
    <TurnstileWidget siteKey={turnstileSiteKey} onToken={onTurnstile}/>{error?<p className="form-error" role="alert">{error}</p>:null}<button className="button dark-button checkout-submit" type="submit" disabled={loading}>{loading?"Checking stock…":"Continue to secure payment"}<ArrowUpRight width={17}/></button><p className="checkout-fine">Payments run through the configured payment provider. Live fulfillment stays disabled unless the deployment explicitly enables a configured provider.</p>
