...
export function FeaturedEdit({ products }: { products: Product[] }) {
  ...
  return <section className="featured-edit section-shell">
    <header className="editorial-heading"><p className="kicker">The current edit · 01—03</p><h2 data-reveal="up">Three ways to<br/><em>stay remembered.</em></h2><div data-reveal="up" data-reveal-delay="0.08"><p>Fragrance chosen by atmosphere, not category. Start with what you want the room to feel like after you leave it.</p><Link href="/shop" className="inline-arrow">See the full collection <ArrowUpRight width={17}/></Link></div></header>
    <div className="editorial-products">
      {featured.map((product,index) => <ProductCard key={product.id} product={product} index={index} feature={index===0} parallax={[0.06, 0.11, 0.15][index]} reveal={index===0 ? "left" : "right"} revealDelay={index===0 ? 0 : 0.06}/>) }
      <div className="editorial-script" data-reveal="up" data-reveal-delay="0.12" aria-hidden="true">
