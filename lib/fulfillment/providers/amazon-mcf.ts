...
export const amazonMcfProvider: FulfillmentProvider = {
  ...
  async quote(request) {
    ...
    const raw = await amazonRequest<Record<string, any>>(`${BASE}/previews`, { method: "POST", body: JSON.stringify(body) });
    const options = (raw?.fulfillmentPreviews || raw?.previews || raw?.offers || []) as Array<Record<string, any>>;
  ...
  async create(request) {
    ...
    const raw = await amazonRequest<Record<string, any>>(`${BASE}/orders`, { method: "POST", body: JSON.stringify(body) });
  ...
  async getStatus(providerOrderId) {
    const raw = await amazonRequest<Record<string, any>>(`${BASE}/orders/${encodeURIComponent(providerOrderId)}`);
    const shipment = raw?.packages?.[0] || raw?.shipments?.[0] || {};
  ...
  async syncInventory(providerSkus): Promise<InventorySnapshot[]> {
    ...
    const raw = await amazonRequest<Record<string, any>>(path);
    const summaries = (raw?.payload?.inventorySummaries || raw?.inventorySummaries || []) as Array<Record<string, any>>;
